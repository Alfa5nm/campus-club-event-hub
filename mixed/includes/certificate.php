...
function write_certificate_pdf(array $record, string $absolutePath): void
{
    $lines = [
        ...
        'Verify: ' . app_url('rifat/verify-certificate.php') . '?code=' . urlencode($record['verification_code']),
    ];
    ...
    if (file_put_contents($absolutePath, $pdf) === false) {
        throw new RuntimeException('Certificate file could not be written.');
    }
}
...
function issue_certificate(int $attendanceId): array
{
    ...
    if (!$record || $record['attendance_status'] !== 'Present') {
        throw new RuntimeException('Only present attendees can receive certificates.');
    }
    $existing = db()->prepare('SELECT * FROM certificate WHERE attendance_id=?');
...
{
    if ($registrationId < 1 || !in_array($status, ['Present','Absent'], true)) {
        throw new InvalidArgumentException('Invalid attendance request.');
    }
    $event = db()->prepare('SELECT club_id,title FROM events WHERE event_id=?');
    $event->execute([$eventId]);
    $eventRow = $event->fetch();
    if (!$eventRow) {
        throw new RuntimeException('Event not found.');
    }
    if (!can_manage_club((int)$eventRow['club_id'])) {
        throw new DomainException('You are not authorized to manage this event.');
    }
    ...
    if (!$registration) {
        throw new RuntimeException('That registration does not belong to this event.');
    }
    if ($registration['registration_status'] === 'Cancelled') {
        throw new RuntimeException('Cancelled registrations cannot be checked in.');
    }
    $markerId = null;
    if (!is_admin()) {
        $m = db()->prepare("SELECT membership_id FROM club_membership WHERE student_user_id=? AND club_id=? AND approval_status='Approved' AND membership_status='Active' LIMIT 1");
        $m->execute([(int)user()['user_id'],(int)$eventRow['club_id']]);
        $markerId = $m->fetchColumn() ?: null;
    }
    db()->beginTransaction();
    try {
        ...
        $a = db()->prepare('SELECT attendance_id FROM attendance WHERE registration_id=?');
        $a->execute([(int)$registration['registration_id']]);
        $attendanceId = (int)$a->fetchColumn();
        ...
        $certificate = null;
        if ($status === 'Present') {
            $previousCertificate = db()->prepare('SELECT status FROM certificate WHERE attendance_id=?');
            $previousCertificate->execute([$attendanceId]);
            $previousCertificateStatus = $previousCertificate->fetchColumn();
            $certificate = issue_certificate($attendanceId);
            if ($registration['registration_status'] !== 'Attended' || $previousCertificateStatus !== 'Active') {
                notify_user((int)$registration['student_user_id'], 'Certificate issued', 'Your certificate for '.$eventRow['title'].' is ready to download.');
            }
        } else {
            $c = db()->prepare("UPDATE certificate SET status='Revoked' WHERE attendance_id=? AND status='Active'");
            $c->execute([$attendanceId]);
            if ($registration['registration_status'] !== 'Absent' || $c->rowCount()) {
                notify_user((int)$registration['student_user_id'], 'Attendance updated', 'You were marked absent for '.$eventRow['title'].($c->rowCount() ? ' Your certificate was revoked.' : ''));
            }
        }
        db()->commit();
        $counts = db()->prepare("SELECT SUM(er.registration_status='Registered') pending,SUM(er.registration_status='Attended') present,SUM(er.registration_status='Absent') absent,COUNT(*) total FROM event_registration er WHERE er.event_id=? AND er.registration_status!='Cancelled'");
