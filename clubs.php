...
?>
<section class="page-shell"><div class="page-head"><div><span class="eyebrow">Campus communities</span><h2>Find where you belong</h2><p>Explore active student organizations or manage the clubs assigned to you.</p></div><?php if(is_admin()):?><a class="button button-primary" href="clubs.php?new=1">Create club</a><?php endif;?></div>
...
<div class="grid"><?php foreach($clubs as $club):?><article class="card"><span class="card-tag"><?=e($club['category'])?></span><h3><?=e($club['club_name'])?></h3><p class="muted"><?=e($club['description'])?></p><div class="detail-list"><span><?= (int)$club['member_count']?> active members</span><span><?= (int)$club['event_count']?> events · <?=e($club['status'])?></span></div><div class="card-footer"><span class="muted"><?=e($club['contact_information'])?></span><div class="actions"><?php if(user()&&!in_array((int)$club['club_id'],$membershipClubIds,true)&&$club['status']==='Active'):?><form method="post"><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="request_join"><input type="hidden" name="club_id" value="<?=$club['club_id']?>"><button class="button button-secondary">Request to join</button></form><?php endif;?><?php if(user()&&can_manage_club((int)$club['club_id'])):?><a class="button button-quiet" href="clubs.php?edit=<?=$club['club_id']?>">Edit</a><?php endif;?></div></div></article><?php endforeach;?></div></section>
