...
<section class="page-shell">
    <div class="page-head">
        ...
        <?php if (is_admin()): ?><a class="button button-primary" href="clubs.php?new=1">＋ Create club</a><?php endif; ?>
    </div>
    ...
    <?php if ((isset($_GET['new']) && is_admin()) || $editClub): ?>
        <form class="card editor-card" method="post" enctype="multipart/form-data">
            ...
            <button class="button button-primary">Save club</button> <a class="button button-quiet" href="clubs.php">Cancel</a>
        </form>
    <?php endif; ?>
    ...
    <div class="grid dynamic-grid" data-filter-grid>
        <?php foreach ($clubs as $index => $club): ?>
            <article class="card club-card reveal" data-filter-item data-category="<?= e(strtolower($club['category'])) ?>" data-search="<?= e(strtolower($club['club_name'].' '.$club['description'].' '.$club['category'])) ?>">
                <div class="club-cover"><img src="<?= e($club['logo'] ?: $fallbackImages[$index % count($fallbackImages)]) ?>" alt="<?= e($club['club_name']) ?>"><span class="cover-index"><?= str_pad((string) ($index + 1), 2, '0', STR_PAD_LEFT) ?></span></div>
                <div class="card-content">
                    ...
                    <div class="card-footer"><span class="badge" data-membership-state><?= e($club['status']) ?></span><div class="actions">
                        <a class="button button-quiet" href="gallery.php?club_id=<?= $club['club_id'] ?>">Gallery</a>
                        ...
                        <?php if (user() && can_manage_club((int) $club['club_id'])): ?><a class="button button-quiet" href="clubs.php?edit=<?= $club['club_id'] ?>">Edit</a><?php endif; ?>
                    </div></div>
                </div>
            </article>
        <?php endforeach; ?>
