...
<div class="grid dynamic-grid" data-filter-grid><?php foreach($clubs as $club):?><article class="card club-card reveal" data-filter-item data-category="<?=e(strtolower($club['category']))?>" data-search="<?=e(strtolower($club['club_name'].' '.$club['description'].' '.$club['category']))?>"><div class="club-monogram"><?=e(strtoupper(substr($club['club_name'],0,1)))?></div><span class="card-tag"><?=e($club['category'])?></span><h3><?=e($club['club_name'])?></h3><p class="muted clamp"><?=e($club['description'])?></p><div class="club-metrics"><span><strong><?= (int)$club['member_count']?></strong> members</span><span><strong><?= (int)$club['event_count']?></strong> events</span></div><div class="card-footer"><span class="badge"><?=e($club['status'])?></span><div class="actions"><?php if(user()&&!in_array((int)$club['club_id'],$membershipClubIds,true)&&$club['status']==='Active'):?><form method="post"><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="request_join"><input type="hidden" name="club_id" value="<?=$club['club_id']?>"><button class="button button-secondary">Request to join</button></form><?php endif;?><?php if(user()&&can_manage_club((int)$club['club_id'])):?><a class="button button-quiet" href="clubs.php?edit=<?=$club['club_id']?>">Edit</a><?php endif;?></div></div></article><?php endforeach;?></div><div class="empty filter-empty" hidden>No clubs match those filters.</div></section>
