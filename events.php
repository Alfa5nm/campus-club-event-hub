...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        $title=trim($_POST['title']??'');$date=$_POST['event_date']??'';$venue=trim($_POST['venue']??'');$capacity=(int)($_POST['maximum_participants']??0);
        if($title===''||!$date||$venue===''||$capacity<1){$error='Enter a title, date, venue, and capacity greater than zero.';}
        else{
            if($eventId){$stmt=db()->prepare('SELECT club_id FROM events WHERE event_id=?');$stmt->execute([$eventId]);if(!can_manage_club((int)$stmt->fetchColumn())){http_response_code(403);exit('Not authorized.');}db()->prepare('UPDATE events SET club_id=?,created_by_user_id=?,title=?,event_date=?,venue=?,maximum_participants=? WHERE event_id=?')->execute([$clubId,$currentUserId,$title,$date,$venue,$capacity,$eventId]);flash('success','Event updated.');}
            else{db()->prepare("INSERT INTO events (club_id,created_by_user_id,title,description,event_category,event_date,start_time,end_time,venue,maximum_participants,registration_deadline,status) VALUES (?,?,?,'Event details will be shared soon.','General',?,'09:00:00',NULL,?,?,?,'Upcoming')")->execute([$clubId,$currentUserId,$title,$date,$venue,$capacity,$date]);flash('success','Event created and published.');}
            redirect('events.php');
        }
    }
}
