...
foreach ($minimums as $table => $minimum) {
    expect((int)db()->query("SELECT COUNT(*) FROM `$table`")->fetchColumn() >= $minimum, "seed includes substantial $table data");
}
...
foreach (db()->query("SELECT file_path FROM certificate WHERE status='Active'")->fetchAll(PDO::FETCH_COLUMN) as $seedFile) {
    expect(is_file(dirname(__DIR__).'/'.$seedFile), 'seeded active certificate file exists');
}
